...
def create_app(test_config=None):
    ...
    def add_question () :
        ...
        try:
            ...
            new_question = Question(question = res["question"], answer = res["answer"], category = res["category"], difficulty = res["difficulty"])
    ...
    def search_ques() :
        ...
        end_page = start_page + QUESTIONS_PER_PAGE
        ques_result = []
        questions_query_search = Question.query.filter(Question.question.ilike('%' + term + '%')).all()
        formatted_questions = [question_query.format() for question_query in questions_query_search]
        categories_query = Category.query.all()
        cat_result = {}
        for category_query in categories_query:
            cat_result[category_query.id] = category_query.type
    ...
    def quizizz():
        ...
        all_ques_ind = [question_query.id for question_query in questions_query]
        new_ques_ind = list(set(all_ques_ind) - set(prev_ques_ind))
        if new_ques_ind == []:
            question = ""
        else:
            current_question = random.choice(new_ques_ind)
            get_current_question = Question.query.filter(Question.id == int(current_question)).first()
            question = {
